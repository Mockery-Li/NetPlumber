...
void NetPlumber::set_node_pipelines(Node *n) {
...
#ifdef __DEBUG__
          printf("Potential next rules\n");
#endif
          array_t *pipe_arr = array_isect_a((*it)->match, n->inv_match, length);
        if (pipe_arr) {
#ifdef __DEBUG__
            printf("Next rule intersect not empty\n");
#endif
  ...
  for (uint32_t i = 0; i < n->input_ports.size; i++) {
#ifdef __DEBUG__
      printf("set backward %d\n", n->input_ports.list[i]);
#endif
...
#ifdef __DEBUG__
  printf("add_source_probe set pipeline done\n");
#endif
